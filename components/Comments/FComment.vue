  ...
  <div class="flex justify-between items-start dark:text-white">
    <div class="flex flex-col space-y-3">
      <div class="flex items-center">
        <strong class="text-lg lg:text-xl">{{ comment.sender.fullName }}</strong>
        <div class="bg-black dark:bg-white w-px h-6 mx-6 opacity-30"></div>
        <div class="rounded-lg border-2 border-brandOrange flex space-x-2 space-x-reverse px-2 py-1 items-center opacity-80">
          ...
          <span class="font-bold text-xs text-brandOrange dark:text-white">{{comment.score}}</span>
        </div>
      </div>
      <div class="flex items-center space-x-4 space-x-reverse">
        <small class="opacity-70 font-light">{{comment.persianDate}}</small>
        <svg width="5" height="5" viewBox="0 0 5 5" fill="none" xmlns="http://www.w3.org/2000/svg" v-if="comment.isConsumer">
          <rect opacity="0.4" width="5" height="5" rx="3" fill="currentColor"/>
        </svg>
        <span class="bg-[#FFF1DA] rounded-full py-1 px-4 text-brandOrange text-xs" v-if="comment.isConsumer">
          خریدار
        </span>
      </div>
    </div>
