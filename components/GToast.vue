...
<template>
  <div class="pointer-events-none">
    <TransitionGroup name="toast" tag="ul" class="flex flex-col gap-2">
      <li v-for="toast in toastStore.messages" :key="toast.id" class="flex items-center p-4 bg-white drop-shadow-md border rounded-xl md:min-w-[330px] gap-4 pointer-events-auto">
        ...
        <div class="flex flex-col gap-2">
          ...
          <div class="flex items-center gap-2 font-light text-xs" v-if="toast.type == ToastType.warning">
            <button class="px-3 py-1 min-w-14 text-center flex items-center justify-center rounded-md bg-danger/20 text-danger hover:bg-danger/80 hover:text-white transition-colors duration-300" @click="toastStore.callHandler(toast.id)">تایید</button>
