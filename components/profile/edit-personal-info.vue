...
<div>
  ...
  <Form @submit="EditInfo" class="mt-4 lg:p-4">
    <div class="grid grid-cols-2 gap-4">
      <base-f-input label="نام" name="name" id="name" is-required v-model="editUserCommand.firstName"/>
      <base-f-input label="نام خانوادگی" name="lastName" id="lastName" is-required v-model="editUserCommand.lastName" />
      <div class="col-span-full">
        <base-f-input label="کدملی" name="nationalId" id="nationalId" is-required :max-length="11" v-model="editUserCommand.nationalId"/>
      </div>
    </div>
    <div class="w-full md:w-2/3 lg:w-1/3 mr-auto mt-4">
      <base-f-button w-full color="primary" :loading="isLoading" type="submit" >
        ثبت تغییرات
      </base-f-button>
    </div>
  </Form>
