  ...
  <div>
    ...
    <table class="dark:text-white mt-12 w-full" v-if="properties.length > 0">
      ...
      <tr v-for="p in properties" :key="p.id">
        ...
        <td class="px-4 py-5">{{p.hintValue}}</td>
...
import {DeleteProperty, GetProperties} from "~/services/product.service";
...
const selectedProperties:Ref<PropertyDto | null> = ref(null);
...
const getData = async ()=>{
  ...
  const result = await GetProperties({pageId:pageId.value,take:10,search:''});
  if(result.isSuccess){
...
const editProperty = (property:PropertyDto) =>{
