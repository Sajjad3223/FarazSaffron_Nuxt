<template>
  <div>
    ...
    <div>
      <ul class="relative w-full flex flex-col bg-gray-100 p-5 rounded-xl min-h-[50vh] max-h-[60vh] overflow-y-auto space-y-4">
        ...
        <li :class="['bg-white flex flex-col space-y-2 p-4 w-1/2 border text-sm font-light',{'self-end':message.sender.userId == accountStore.currentUser.id}]"
        ...
        <li class="absolute inset-x-4 bottom-4 p-4 bg-white rounded-lg flex gap-2">
          <base-g-input label="پیام ..." required v-model="sendMessageData.text"/>
...
import {GetTicket, SendTicketMessage, SendTicketMessageByAdmin} from "~/services/ticket.service";
import {type TicketDto} from "~/models/ticket/ticketQueries";
...
const ticket:TicketDto = data.value?.data!;
console.log(ticket)
