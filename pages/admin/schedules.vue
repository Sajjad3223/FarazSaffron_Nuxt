<template>
  <div class="w-full overflow-hidden">
    <Head>
      <Title>اطلاع رسانی های زمان بندی شده</Title>
    </Head>
    <base-f-divider :logo-divider="false" title="اطلاع رسانی های زمان بندی شده">
      <template #left>
        ...
        <table class="w-full whitespace-no-wrap">
          ...
          <template v-for="q in schedules">
            <tr class="text-gray-700 ">
              ...
              <td class="px-4 py-3 text-sm text-nowrap max-w-[200px] truncate">
                {{ new Date(q.scheduleDate).toLocaleDateString('fa-Ir') }}
              </td>
              <td class="px-4 py-3">
...
const selectedSchedule: Ref<ScheduleDto | null> = ref(null);
...
const getData = async () => {
  ...
  const result = await GetSchedules({pageId:pageId.value,take:10});
  if (result.isSuccess) {
