<template>
  <div class="w-full overflow-hidden">
    ...
    <base-f-divider :logo-divider="false" title="تنظیمات صفحه اصلی">
      <template #left>
        <base-f-button color="primary" @clicked="showAddTemplateModal = true" text-color="white">
          الگوی جدید
        </base-f-button>
      </template>
    </base-f-divider>
    ...
    <div v-if="loading" class="w-full grid place-items-center -translate-y-[200%] my-10">
      <GLoadingProgress />
    </div>
    <div v-else class="w-full">
      <div class="flex flex-col space-y-5 mt-5" v-if="templates.length > 0">
        <div class="w-full flex items-center justify-between  rounded-lg p-4" v-for="template in templates">
          <div class="flex gap-4 items-center">
            <!--  Active Template  -->
            <button class="text-black/50 hover:text-black/70 transition-colors duration-300 px-2 py-1 rounded-md" @click="setActiveTemplate(template.id)" v-if="!template.isActive" title="تنظیم به عنوان الگوی فعال">
            ...
            <button class="text-black/50 hover:text-black/70 transition-colors duration-300 px-2 py-1 rounded-md" v-else disabled title="الگوی فعال">
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
