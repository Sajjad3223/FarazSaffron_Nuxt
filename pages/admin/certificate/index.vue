<template>
  <div>
    <Head>
      <Title>شناسنامه ها</Title>
    </Head>
    <base-f-divider :logo-divider="false" title="شناسنامه ها">
      <template #left>
        <base-f-button color="primary" @clicked="showCreateCertificateModal = true" text-color="white">
          شناسنامه جدید
        </base-f-button>
      </template>
    </base-f-divider>

    <base-g-modal title="افزودن شناسنامه جدید" v-model="showCreateCertificateModal">
      <admin-certificates-add-certificate @certificate-created="showCreateCertificateModal = false,getData()" />
    </base-g-modal>

    <base-g-modal title="ویرایش شناسنامه" v-model="showEditCertificateModal">
      <admin-properties-edit-c-property :property="selectedCertificate" @propertyEdited="showEditCertificateModal = false,getData()" />
    </base-g-modal>

    <table class=" mt-12 w-full" v-if="certificates.length > 0">
      ...
      <tr v-for="c in certificates" :key="c.id">
        <td class="px-4 py-5">
          <NuxtLink :to="`/admin/certificate/${c.id}`">
            {{c.title}}
          </NuxtLink>
        </td>
        <td class="px-4 py-5">{{c.isActive ? 'فعال' : 'غیر فعال'}}</td>
        <td class="px-4 py-5">{{c.persianDate}}</td>
        <td class="px-4 py-5">
          <div class="flex gap-2">
            <!--  Edit Certificate  -->
            <button class="bg-warning/30 text-warning hover:bg-warning transition-colors duration-300 hover:text-white px-2 py-1 rounded-md top-2 left-2" @click="editCertificate(c)">
            ...
            <!--  Delete Certificate  -->
            <button class="bg-danger/30 text-danger hover:bg-danger transition-colors duration-300 hover:text-white px-2 py-1 rounded-md top-2 left-2" @click="deleteCertificate(c.id)">
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
import type {CAPropertyDto, CertificateFilterData} from "~/models/certificate/authenticatorDto";
import {DeleteCertificate, GetAllCertificates} from "~/services/certificate.service";
...
const selectedCertificate:Ref<CAPropertyDto | null> = ref(null);
...
const getData = async ()=>{
  ...
  const result = await GetAllCertificates({pageId:pageId.value,take:10,search:''});
  if(result.isSuccess){
...
const editCertificate = (property:CAPropertyDto) =>{
  selectedCertificate.value = property;
  showEditCertificateModal.value = true;
}

const deleteCertificate = (certificateId:number) =>{
  toast.showToast("آیا از حذف شناسنامه اطمینان دارید؟",ToastType.warning,0)
      .then(async res=>{
        if(res.isConfirmed){
