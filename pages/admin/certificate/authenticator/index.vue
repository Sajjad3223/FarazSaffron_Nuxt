      ...
      <tr v-for="a in authenticators" :key="a.id">
        ...
        <td class="px-4 py-5">
          <div class="flex gap-2">
            ...
            <!--  Delete Property  -->
            <button class="bg-danger/30 text-danger hover:bg-danger transition-colors duration-300 hover:text-white px-2 py-1 rounded-md top-2 left-2" @click="deleteAuthenticator(a.id)">
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const getData = async ()=>{
  ...
  const result = await GetAllAuthenticators({pageId:pageId.value,take:10,search:''});
  if(result.isSuccess){
