<template>
  <div>
    <Head>
      <Title>ویژگی های اصالت نامه ها</Title>
    </Head>
    <base-f-divider :logo-divider="false" title="ویژگی های اصالت نامه ها">
      <template #left>
    ...
    <base-g-modal title="افزودن ویژگی جدید" v-model="showCreatePropertyModal">
      <admin-properties-create-a-property @propertyCreated="showCreatePropertyModal = false,getData()" />
    </base-g-modal>

    <base-g-modal title="ویرایش ویژگی" v-model="showEditPropertyModal">
      <admin-properties-edit-a-property :property="selectedProperties" @propertyEdited="showEditPropertyModal = false,getData()" />
    </base-g-modal>
      ...
      <tr v-for="p in properties" :key="p.id">
        ...
        <td class="px-4 py-5">
          <div class="flex gap-2">
            <!--  Edit Property  -->
            <button class="bg-warning/30 text-warning hover:bg-warning transition-colors duration-300 hover:text-white px-2 py-1 rounded-md top-2 left-2" @click="editProperty(p)">
            ...
            <!--  Delete Property  -->
            <button class="bg-danger/30 text-danger hover:bg-danger transition-colors duration-300 hover:text-white px-2 py-1 rounded-md top-2 left-2" @click="deleteProperty(p.id)">
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
import {DeleteAProperty, GetAProperties} from "~/services/certificate.service";
...
const getData = async ()=>{
  ...
  const result = await GetAProperties(0,{pageId:pageId.value,take:10,search:''});
  if(result.isSuccess){
...
const deleteProperty = (propertyId:number) =>{
  toast.showToast("آیا از حذف ویژگی اطمینان دارید؟",ToastType.warning,0)
      .then(async res=>{
        if(res.isConfirmed){
          const result = await DeleteAProperty(propertyId);
          if(result.isSuccess){
            toast.showToast()
            await getData();
          }
