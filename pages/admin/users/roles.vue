...
import type {RoleFilterData, RoleFilterParams} from "~/models/role/roleQueries";
...
const filterParams:RoleFilterParams = reactive({
  pageId:pageId,
  take:10,
  search:''
})
...
const getData = async ()=>{
  ...
  const result = await GetRoles(filterParams);
  if(result.isSuccess){
